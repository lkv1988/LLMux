...
**热更新**：编辑 `config.json5`，更改立即生效，无需重启。
...
<img src="./assets/dashboard-screenshot.png" alt="LLMux 监控面板" width="800">

监控面板显示：
- 汇总卡片：总请求数、累计成本、平均 TTFB
- 供应商状态：实时健康指示器和冷却计时器
- Token 用量表：按模型统计，带可配置窗口活动趋势图（默认 4 小时）
- 图表：小时/每日 token 趋势、模型分布饼图、缓存命中率对比
- 时间范围选择器：今天 / 7 天 / 30 天

启动代理后访问 `http://localhost:34250/dashboard`。在 macOS 上，代理启动时会自动以 Chrome 应用模式（独立窗口）打开监控面板。
...
使用你的 API 密钥编辑 `config.json5`。这是一个将 Sonnet 路由到多个供应商的示例（你可以配置任何你想要的路由策略）：
...
API 密钥可以是任何有效格式——代理会忽略它并使用 `config.json5` 中的密钥。

### 4. 打开监控面板

在 macOS 上，代理启动时会自动以 Chrome 应用模式打开监控面板。否则，访问 `http://localhost:34250/dashboard` 实时查看请求流。
...
| `activity.windowMinutes` | number | 240 | 活动趋势图时间窗口（分钟） |
| `activity.bucketMinutes` | number | 10 | 活动数据桶大小（分钟） |
| `activity.pushIntervalMs` | number | 5000 | 活动数据推送到面板的间隔（毫秒） |
...
**热配置重载**：通过 `fs.watch` 检测 `config.json5` 的更改并立即应用——重载时清除所有供应商冷却记录。
...
**我当前的速率是多少？**
- 每桶和每窗口的 tokens——追踪重度编码会话期间的突发活动（可配置时间窗口）

**活动模式：**
- 每个模型的可配置窗口趋势图（默认 4 小时）——可视化一段时间内的请求分布
- 小时趋势图——识别高峰使用时段
...
- **统计持久化**：Token 统计保存到 `data/token_stats.json`（每 2 秒防抖写入）；活动数据保存到 `data/activity_data.json`（关闭时持久化）
